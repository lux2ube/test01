...
- **Financial Reports System:** Comprehensive admin reporting with filter-based queries. Features: (1) Record type selection (cashback, referral commissions, deposits, withdrawals completed/processing, orders), (2) Cascading filters (user → broker → account), (3) Date range filtering (from/to), (4) Data only displays after clicking "Confirm Filter" button. Queries confirmed records from `cashback_transactions` table (all records are confirmed), `transactions` table with type='referral_commission' for referrals, and `withdrawals` table with appropriate status filters.
