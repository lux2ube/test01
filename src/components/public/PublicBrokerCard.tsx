...
import { Star, ChevronLeft, Banknote, Shield, ExternalLink, Gift } from "lucide-react";
...
interface PublicBrokerCardProps {
  broker: Broker;
}

export function PublicBrokerCard({ broker }: PublicBrokerCardProps) {
  if (!broker) {
    return null; 
  }
  ...
  const brokerRegistrationUrl = broker.instructions?.new_account_link || broker.instructions?.link || '';
                ...
                <div className="grid grid-cols-2 gap-2">
                    {brokerRegistrationUrl ? (
                        <Button asChild variant="outline" size="sm" className="font-medium h-8 text-xs">
                            <a href={brokerRegistrationUrl} target="_blank" rel="noopener noreferrer">
                    ...
                    <Button asChild size="sm" className="font-medium h-8 text-xs">
                        <Link href={`/login?next=/dashboard/brokers/${broker.id}/link`}>
